    ...
    <section class="mb-8 flex w-full items-center space-x-6">
      <div class="rounded-xl flex-1 p-8 shadow-lg">
        <fieldset class="fieldset w-full">
          <legend class="fieldset-legend text-2xl font-semibold text-gray-800 mb-6"
            >Thêm Phiếu Giảm Giá
          </legend>
      ...
      <div
        v-if="isCustomerSelectionEnabled"
        class="rounded-xl flex-1 flex-col p-8 shadow-lg bg-white border border-gray-300"
      >
          ...
          <div class="mt-4">
            ...
            <div class="flex gap-6 mt-6">
              <Button
                @click="applyFilter"
                label="Lọc"
                class="p-button-lg p-button-primary w-full rounded-md shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
              />
            </div>
          </div>
    ...
    try {
      ...
      if (voucherData) {
        ...
        if (voucherData.phieuRiengTu && Array.isArray(voucherData.danhSachNguoiDung)) {
          ...
          const selectedCustomersPromises = selectedIds.map((id) =>
            customerStore.fetchCustomerById(id),
          )
  ...
  try {
    ...
    if (isCustomerSelectionEnabled.value && selectedCustomers.value.length === 0) {
      toast.add({
        severity: 'error',
        summary: 'Lỗi',
        detail: 'Vui lòng chọn ít nhất một khách hàng khi phiếu giảm giá là riêng tư!',
        life: 3000,
      })
      return
    }
    ...
    const customerIds = isCustomerSelectionEnabled.value
      ? selectedCustomers.value.map((user) => user.id)
      : []
    ...
    toast.add({
      severity: 'error',
      summary: 'Lỗi',
      detail: 'Không thể thêm hoặc cập nhật phiếu giảm giá!',
      life: 3000,
    })
  }
}
