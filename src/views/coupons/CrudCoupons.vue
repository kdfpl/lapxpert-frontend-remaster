<template>
    <section class="flex h-full w-full flex-col max-h-[500px] overflow-y-auto p-6">
        <section class="mb-8 flex w-full items-center space-x-6">
            <div class="rounded-xl flex-1 p-8 shadow-lg">
                <fieldset class="fieldset w-full">
                    <legend class="fieldset-legend text-2xl font-semibold text-gray-800 mb-6">Thêm Phiếu Giảm Giá</legend>

                    <label class="fieldset-label">Mã Voucher:</label>
                    <InputText v-model="voucher.maPhieuGiamGia" class="w-full p-inputtext-l" placeholder="Mã voucher..." />

                    <label class="fieldset-label">Loại Giảm Giá:</label>
                    <Select v-model="voucher.loaiPhieuGiamGia" :options="discountTypeOptions" class="w-full p-dropdown-sm" optionLabel="label" optionValue="value" />

                    <label class="fieldset-label">Giá Trị:</label>
                    <InputNumber v-model="voucher.giaTriGiam" class="w-full p-inputnumber" placeholder="100.000..." mode="decimal" />

                    <label class="fieldset-label">Điều Kiện:</label>
                    <InputNumber v-model="voucher.giaTriDonHangToiThieu" class="w-full p-inputnumber" placeholder="100.000..." mode="decimal" />

                    <div class="flex gap-6 mt-6">
                        <fieldset class="fieldset flex-1">
                            <label class="fieldset-label">Ngày Bắt Đầu</label>
                            <Calendar v-model="voucher.ngayBatDau" class="w-full p-calendar" @input="handleDateChange" />
                        </fieldset>

                        <fieldset class="fieldset flex-1">
                            <label class="fieldset-label">Ngày Kết Thúc</label>
                            <Calendar v-model="voucher.ngayKetThuc" class="w-full p-calendar" @input="handleDateChange" />
                        </fieldset>
                    </div>

                    <label v-if="!isCustomerSelectionEnabled" class="fieldset-label">Số Lượng:</label>
                    <InputNumber v-if="!isCustomerSelectionEnabled" v-model="voucher.soLuongBanDau" class="w-full p-inputnumber" placeholder="Số lượng..." />

                    <div class="flex items-center gap-2 mt-6">
                        <Checkbox :binary="true" v-model="isCustomerSelectionEnabled" @change="handlePrivateSelection" />
                        <label class="text-sm text-gray-700">Riêng tư (Chỉ dành cho khách hàng được chọn)</label>
                    </div>

                    <div class="mt-6 flex gap-6">
                        <Button v-if="voucher.id !== null" @click="submitVoucher" label="Cập Nhật" class="p-button-lg p-button-primary w-full" />
                        <Button v-else @click="submitVoucher" label="Thêm" class="p-button-lg w-full" severity="success" variant="outlined" />
                        <Button @click="cancel" label="Hủy" class="p-button-lg w-full" severity="danger" variant="outlined" />
                    </div>
                </fieldset>
            </div>
            ...
            <div v-if="isCustomerSelectionEnabled" class="rounded-xl flex-1 flex-col p-8 shadow-lg">
                <span class="text-2xl font-bold text-gray-800 mb-4">Chọn Khách Hàng</span>
                <div class="max-h-[600px] overflow-auto">
                    <DataTable :value="customerList" selectionMode="checkbox" v-model:selection="selectedCustomers" @selection-change="handleSelectionChange" class="w-full p-datatable-sm">
                        <Column selectionMode="multiple" style="width: 3rem" />
                        <Column field="hoTen" header="Tên Khách Hàng" style="width: 13rem" />
...
import { InputText, InputNumber, Calendar, Checkbox, Button, DataTable, Column, Select } from 'primevue';
...
const handlePrivateSelection = () => {
    if (isCustomerSelectionEnabled.value && selectedCustomers.value.length === 0) {
        toast.add({ severity: 'error', summary: 'Lỗi', detail: 'Vui lòng chọn ít nhất một khách hàng!', life: 3000 });
        return;
    }
...
const submitVoucher = async () => {
    try {
        ...
        toast.add({ severity: 'success', summary: 'Thành công', detail: 'Đã lưu phiếu giảm giá!', life: 3000 });
        router.push('/discounts/coupons');
    } catch (error) {
        console.error('Lỗi submitVoucher:', error);
        toast.add({ severity: 'error', summary: 'Lỗi', detail: 'Không thể lưu phiếu giảm giá.', life: 3000 });
    }
};
